...
export function LogMediatorProperty(props: Props) {
  ...
  const handleLogSeparatorChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    setLogSeparator(event.target.value);
  };
  const handlePropertiesChange = async (event: React.ChangeEvent<HTMLTextAreaElement>) => {
  ...
  const handleSubmit = async () => {
    ...
    let snippetCompletionResponse: SnippetCompletionResponse = await getSnippetCompletion(selectedLogLevel, selectedVersionType, logSeparator, properties, description, langClient);
                    ...
                    <Form>
                        <Form.Group>
                            ...
                            <OverlayTrigger placement="right" overlay={<Tooltip id="help-tooltip">This parameter is used to specify a value to be used in the log to separate attributes. The comma is default. Can be add tab as "/t" and new line as "/n"</Tooltip>}>
                              ...
                              <Form.Control type="text" value={logSeparator} onChange={handleLogSeparatorChange} placeholder=" " />
                            <Form.Label className='Properties'>Properties</Form.Label>
                            {/* When a user clicks this textbox, the LogProperty Model appears.*/}
                            <Form.Control as='textarea' value={properties} onChange={handlePropertiesChange} readOnly />
                            ...
                            <Form.Control as="textarea" value={description} onChange={handleDescriptionChange} placeholder="eg: None" />
                        </Form.Group>
                    </Form>
                </Row>
            </Modal.Body>
